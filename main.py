...
Original file is located at
    https://colab.research.google.com/drive/16Gt0hNASLIStFOYr_ZGo-0PV8wYHQ5lB
"""
import numpy as np
...
# square image so size_of_output*size_of_output
size_of_output=100
...
# Here we just perform the needed reshaping (so that all values are in the [0,1] interval),
# we switch the labels to categorical and perform random shuffle of the input data.
...
# we did not manage to test the network with other optimizers (apart from some initial tests with an sgd(0.1)
# just to see that it actually had convergence issues)
...
# The kind of augmentations depend on the dataset (it does not make much sense to employ vertical_flip for example).
# All parameters (commented or not) have been tested. On the right we say whether they are useful and  for which values.
